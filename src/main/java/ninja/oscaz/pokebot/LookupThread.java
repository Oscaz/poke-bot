...
final class LookupThread extends Thread {
    ...
    private static final String SEARCH_URL_FORMAT = "https://pokemon.gamepedia.com/index.php?search=%s"; // Format to initiate search on Wiki
    ...
    private static final String PAGE_URL_FORMAT = "https://pokemon.gamepedia.com%s"; // Format to convert relative page URL  into fully declared URL
    private static final String PAGE_TITLE_CSS_SELECTOR = "#firstHeading"; // CSS Selector to find Wiki page's title
    private static final String FIRST_ELEMENT_CSS_SELECTOR_PREFIX = "#mw-content-text > "; // div > div > p:first-of-type"; // CSS Selector to locate description of Wiki page
    private static final String FIRST_ELEMENT_CSS_SELECTOR_P1 = LookupThread.FIRST_ELEMENT_CSS_SELECTOR_PREFIX + "p:nth-of-type(1)";
    private static final String FIRST_ELEMENT_CSS_SELECTOR_P2 = LookupThread.FIRST_ELEMENT_CSS_SELECTOR_PREFIX + "p:nth-of-type(2)";
    ...
    private String getDescriptionOfWiki(Document page) {
        Elements firstDescription = page.select(LookupThread.FIRST_ELEMENT_CSS_SELECTOR_P1); // Find first description element using CSS Selector
        Elements secondDescription = page.select(LookupThread.FIRST_ELEMENT_CSS_SELECTOR_P2); // Find second description element using CSS Selector
        String separator = firstDescription.text().isEmpty() ? "" : "\n";
        return this.removeCitations(firstDescription.text() + separator + secondDescription.text()); // Return text contained within element without citations
    }
